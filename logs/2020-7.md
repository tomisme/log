...
2. In an extra step, use a zipper to traverse the hiccup and when we hit "modifier" code strip it out and run it, passing in the containing block and replacing it with the codes return value!

This seems super hacky but and the API is garbage (should we use "{{}}" like Roam?) but.. it works? Here's the zipper code:
...
1. What are the security implications of allowing ebedded code (possibly dowloaded plugin code later) access to other parts of your graph?
2. What do we need to pass into the modifier block code so that it can mark the TODO as DONE?
