        ...
        class FolderWatcher {
            ...
            displayFiles(files) {
                ...
                files.forEach(file => {
                    ...
                    fileElement.innerHTML = `
                        ...
                        <span>${this.formatFileSize(file.size)} • ${new Date(file.lastModified).toLocaleTimeString()}</span>
                    `;
            ...
            handleFileChanges(records) {
                records.forEach(record => {
                    ...
                    switch (record.type) {
                        case 'created':
                            message = `📄 NEW FILE: ${record.changedHandle.name}`;
                            this.addFileToList(record.changedHandle);
                            break;
                        case 'modified':
                            message = `✏️ MODIFIED: ${record.changedHandle.name}`;
                            break;
                        case 'moved':
                            message = `📁 MOVED: ${record.relativePathComponents?.join('/') || record.changedHandle.name}`;
                            break;
                        case 'deleted':
                            message = `🗑️ DELETED: ${record.changedHandle.name}`;
                            break;
                        default:
                            message = `🔍 CHANGE: ${record.changedHandle.name} (${record.type})`;
                    }
